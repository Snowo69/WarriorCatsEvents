...
import net.snowteb.warriorcats_events.item.custom.ContainerSlot;
import net.snowteb.warriorcats_events.item.custom.DockBackpackContainer;
...
public class DockBackpackMenu extends AbstractContainerMenu {
    ...
    public DockBackpackMenu(int windowId, Inventory inv, ItemStack backpackStack) {
        super(ModMenuTypes.DOCK_BACKPACK.get(), windowId);

        this.backpackStack = backpackStack;
        this.openingPlayer = inv.player;
        DockBackpackContainer container = new DockBackpackContainer(backpackStack);
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                this.addSlot(new ContainerSlot(container, col + row * 9, 8 + col * 18, 13 + row * 18));
            }
        }
    }
