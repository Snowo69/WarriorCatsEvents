...
public class FeverEffectOverlay {
    ...
    public static final IGuiOverlay HUD_FEVER = ((gui, pGuiGraphics, partialTick, screenWidth, screenHeight) -> {

        LocalPlayer player = Minecraft.getInstance().player;
        Minecraft mc = Minecraft.getInstance();
        if (player == null) return;
        if (mc.level == null) return;
        if (!player.hasEffect(ModEffects.FEVER.get())) return;
        ...
        float pulseAlpha = Mth.clamp(getAlpha(mc, partialTick) * 2, 0.5f, 1f);
    ...
    private static float getDarknessGamma(float pPartialTick, Minecraft mc) {
        if (mc.player.hasEffect(ModEffects.FEVER.get())) {
            MobEffectInstance mobeffectinstance = mc.player.getEffect(ModEffects.FEVER.get());
            if (mobeffectinstance != null && mobeffectinstance.getFactorData().isPresent()) {
                return mobeffectinstance.getFactorData().get().getFactor(mc.player, pPartialTick);
            }
        }
        return 0.0F;
    }
    ...
    private static float getRemainingAlpha(LocalPlayer player) {
        MobEffectInstance effect = player.getEffect(ModEffects.FEVER.get());
