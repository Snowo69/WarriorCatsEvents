    ...
    public InteractionResult interactLivingEntity(ItemStack pStack, Player pPlayer, LivingEntity pInteractionTarget, InteractionHand pUsedHand) {
        ...
        if (pInteractionTarget instanceof Player target && pUsedHand == InteractionHand.MAIN_HAND) {
            if (!pPlayer.level().isClientSide()) {
                ...
                WCEPlayerData.Age myAge = pPlayer.getData(ModAttachments.PLAYER_WCE_DATA).getMorphAge();
                WCEPlayerData.Age targetAge = target.getData(ModAttachments.PLAYER_WCE_DATA).getMorphAge();

                UUID myMate = pPlayer.getData(ModAttachments.PLAYER_WCE_DATA).getMateUUID();
                UUID targetMate = target.getData(ModAttachments.PLAYER_WCE_DATA).getMateUUID();

                String myMorphName = pPlayer.getData(ModAttachments.PLAYER_WCE_DATA).getMorphName();
                String targetMorphName = target.getData(ModAttachments.PLAYER_WCE_DATA).getMorphName();
